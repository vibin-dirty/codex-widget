...
{
    [Fact]
    public void UsageProject_DoesNotReferenceHostSpecificPackagesOrHostProjects()
    {
        var project = LoadProject("src/CodexWidget.Usage/CodexWidget.Usage.csproj");
    ...
    [Fact]
    public void UsageAssembly_DoesNotReferenceUiAssemblies()
    {
        var referencedAssemblyNames = typeof(UsageEndpointResolver).Assembly.GetReferencedAssemblies()
    ...
    [Fact]
    public void UsageSources_DoNotImportHostSpecificNamespacesOrValidationRuntimeCode()
    {
        var repositoryRoot = FindRepositoryRoot();
        var sourceDirectory = Path.Combine(repositoryRoot, "src", "CodexWidget.Usage");
    ...
    [Fact]
    public void UsageProject_CanConsumeCoreContracts()
    {
        Assert.Equal(TimeSpan.Zero, SystemClock.Instance.UtcNow.Offset);
        Assert.Equal(StatusAvailability.Available, StatusAvailability.Available);
    }
